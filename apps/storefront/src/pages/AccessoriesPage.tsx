    ...
    >
      <div className="relative aspect-[4/5] w-full overflow-hidden flex items-center justify-center p-12">
        ...
        <img
          ...
          } ${isInView ? 'scale-105' : 'scale-100'} md:scale-100 md:group-hover:scale-105`}
        />
      </div>
      ...
      <div className="flex flex-col items-center text-center pb-10 px-8 space-y-1">
        ...
        <h3 className="text-lg tracking-tight text-gunmetal italic">
          ...
          {accessory.collection || (accessory.brand ?? 'Accessory')}
        </h3>

        <div className="flex flex-col items-center">
          <p className="text-[11px] uppercase tracking-widest text-bone">{accessory.name}</p>
          <div className={`h-[1px] bg-golden mt-3 transition-all duration-500 ${isInView ? 'w-12' : 'w-4'} md:w-4 md:group-hover:w-12`} />
        </div>

        <div className={`overflow-hidden transition-all duration-500 ${isInView ? 'h-6 opacity-100' : 'h-0 opacity-0'} md:h-0 md:opacity-0 md:group-hover:h-6 md:group-hover:opacity-100`}>
          <p className="text-[12px] text-gunmetal font-branding mt-2">REF. {accessory.ref}</p>
        </div>
      </div>
    </Link>
  );
...
const AccessoriesPage: React.FC = () => {
  ...
  const [isFilterOpen, setIsFilterOpen] = useState(queryParams.get('in_stock') === 'true');
  ...
  useEffect(() => {
    ...
    const fetchFilters = async () => {
      try {
        const [brandData, collectionData, accessoryTypeData] = await Promise.all([
        ...
        setAccessoryTypes(Array.isArray(accessoryTypeData) ? accessoryTypeData : []);
      } catch (err) {
        if (controller.signal.aborted) return;
        console.error('Failed to fetch accessory filters:', err);
      }
    };
  ...
  const renderFilterContent = () => {
    const toggleBrand = (brandId: string | null) => {
      ...
      setSelectedBrands((prevBrands) => {
        ...
        if (prevBrands.includes(brandId)) return [];
        return [brandId];
      });
    };
    ...
    const visibleCollections = collections.filter((collection) => selectedBrands.includes(collection.brand_id));
      ...
      <div className="max-w-[1600px] mx-auto px-6 lg:px-12 flex flex-col lg:flex-row">
        <AnimatePresence>
          {isFilterOpen && (
            ...
            >
              <div className="w-[280px]">{renderFilterContent()}</div>
            </motion.aside>
          )}
        </AnimatePresence>
        ...
        <div className="flex-1 relative min-h-[400px]">
          {isLoading ? (
            ...
            <motion.div
              key={`${selectedBrands.join(',')}-${selectedCollections.join(',')}-${selectedAccessoryTypes.join(',')}-${isInStockOnly}`}
              initial="hidden"
              animate="visible"
              variants={{
                hidden: { opacity: 0 },
